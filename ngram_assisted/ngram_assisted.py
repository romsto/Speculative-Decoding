...
def ngram_assisted_speculative_generate(
    ...
    """
    Generate text sequence using the ngram adapted speculative decoding algorithm.
    Adaptation of Speculative Decoding (https://arxiv.org/pdf/2211.17192.pdf) to use a dynamic ngram model.
    
    Args:
        inputs (List[int]): input sequence of batch size 1.
        drafter (DynaGram): dynamic ngram model.
    ...
    ngram.initialize(input_ids[..., :prompt_len])
    ...
    if first_target:
        ...
        current_position += 1
        ngram.update(input_ids[..., :prompt_len], t)
        if debug:
    ...
    while current_position < total_len:
        ...
        for k in range(corrected_gamma):
            copied_inputs_ids[0, current_position + k], known = ngram.next_token(copied_inputs_ids[..., :current_position + k])
            if not known[0] and stop_if_unknown:
                corrected_gamma = k
                break
            # TODO maybe add the probabilities
        ...
        for i in range(n):
            ngram.update(input_ids[..., :current_position + i], input_ids[..., current_position + i].unsqueeze(0))
            if filler_top_k > 1:
                ngram.update(input_ids[..., :current_position + i], p[..., i, :].topk(filler_top_k).indices)
        ngram.update(input_ids[..., :current_position + n], x)
        if filler_top_k > 1:
            ngram.update(input_ids[..., :current_position + n], p_p.topk(filler_top_k).indices)
